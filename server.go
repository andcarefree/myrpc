...
func (server *Server) Regsiter(rcvr interface{}) error {
	s := newService(rcvr)
...
func Register(rcvr interface{}) error {
	return DefaultServer.Regsiter(rcvr)
}
...
func (server *Server) findService(serviceMethod string) (svc *service, mType *methodType, err error) {
	...
	if !ok {
		err = errors.New("rpc server: can't find service")
		return
	}
	svc = svci.(*service)
	mType = svc.method[methodName]
	if mType == nil {
		err = errors.New("rpc sercer: can't find method " + methodName)
		return
	}
	return
}
...
func (server *Server) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		log.Println("rpc hijacker ", req.RemoteAddr, ": ", err.Error())
	}
